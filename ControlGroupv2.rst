...
プロアクティブ・リクレイム（この
	インターフェイスによってトリガーされる）プロアクティブ・リクレイムは
  ...
  memory.swap.high
	非ルートの
	cgroups に存在する読み書き可能な単一値ファイルです。 デフォルトは "max"。
...
io」コントローラーは、IOリソースの分配を制御する。 この
このコントローラは、ウェイトベースの分配と、絶対的な帯域幅またはIOPS制限の分配の両方を実装している。
制限分配の両方を実装している。
が使用されている場合のみ利用可能であり、blk-mqデバイスではどちらのスキームも利用できない。
blk-mqデバイスではどちらのスキームも利用できません。
...
cgroupライトバックには、基礎となる
ファイルシステムからの明示的なサポートが必要です。 現在、cgroup writeback は ext2、ext4、btrfs、f2fs、および xfs で実装されています、
