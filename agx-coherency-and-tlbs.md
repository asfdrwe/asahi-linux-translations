...
コヒーレントであることを示唆しています。PTE(Page Table Entry)をまだダンプしていませんが、書き込まれた仮想アドレスのアドレス変換(訳注: AT, Address Translation)
...
すぐにGPUフォルトを引き起こすように見えるので、もう確信がありません（GPUはマイクロ秒以内にフォルトし、私はAPの観点からTLBI後にタイル配列書き込みが
