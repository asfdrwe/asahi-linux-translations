...
2026/3/19時点の[u-boot](https://github.com/AsahiLinux/docs/blob/main/docs/sw/u-boot.md)の翻訳
...
もしあなたが Asahi ユーザーで、U-Boot や m1n1 をハックすることに興味がなければ、ディストリビューションによって自動的に管理されます。 
...
Fedora Asahi Remix 内でこれを行う例として、[`uboot`](https://pagure.io/fedora-asahi/uboot-tools) と [`asahi-scripts`](https://src.fedoraproject.org/rpms/asahi-scripts/) パッケージを参照してください。
...
USBを使ってリカバリ/『ライブCD』をロードする場合は、`/etc/dracut.conf` 内の `force_drivers` に `usbhid xhci_hcd` があることを確認してください。
...
注: `poweoff` コマンドは Apple Silicon 搭載のU-Bootでは使用できません。
機器の電源を切るには、OS（Linux または macOS）を起動した後、それぞれのシャットダウンコマンドをご使用ください。
